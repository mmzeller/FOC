const FALLBACK_REPORT = {
  reportDate: "2026-08-10", cutoffLabel: "Monday, Aug 10", summary: "Stored baseline report. Configure the V2 backend to run live research.",
  books: [
...
let CLIENT_TOKEN = String(window.FOC_CLIENT_TOKEN || localStorage.getItem("foc_client_token") || "");
...
function getToken(){
  ...
  const token=window.prompt('Enter your FOC Heat access token. It is stored only in this browser.');
  if(token){CLIENT_TOKEN=token.trim();localStorage.setItem('foc_client_token',CLIENT_TOKEN);return CLIENT_TOKEN;}
  return '';
}
function api(path, options={}){
  const token=getToken();
  if(!token) return Promise.reject(new Error('No FOC Heat access token configured.'));
  return fetch(`${API_BASE}${path}`,{...options,headers:{"Content-Type":"application/json","Authorization":`Bearer ${token}`,...(options.headers||{})}}).then(async r=>{const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(body.error||`Request failed (${r.status})`);return body});
}
function renderBook(b){
  const sources=(b.sources||[]).filter(Boolean).map(u=>`<a href="${esc(u)}" target="_blank" rel="noopener">Source</a>`).join(" ");
  const facts=(b.verifiedFacts||[]).map(x=>`<li>${esc(x)}</li>`).join(""); const evidence=(b.marketEvidence||[]).map(x=>`<li>${esc(x)}</li>`).join(""); const unknowns=(b.unknowns||[]).map(x=>`<li>${esc(x)}</li>`).join("");
...
function render(report, watchlist=null){
  const r=report||FALLBACK_REPORT; document.querySelector('#reportTitle').textContent=`FOC Heat — ${r.reportDate||'Current'}`; document.querySelector('#reportMeta').textContent=`${r.summary||'Investment first.'} ${r.generatedAt?`Generated ${new Date(r.generatedAt).toLocaleString()}.`:''}`; document.querySelector('#cutoff').textContent=r.cutoffLabel||'—'; document.querySelector('#report').innerHTML=(r.books||[]).map(renderBook).join('')||'<div class="empty">No report available.</div>';
  const wl=watchlist || (r.watchlistAdds||[]); document.querySelector('#watchCount').textContent=wl.length; document.querySelector('#watchlist').innerHTML=wl.map(x=>`<div class="watch-item"><strong>${esc(x.title||x[0])}</strong><span>${esc(x.reason||x[1]||'')}</span></div>`).join('')||'<div class="empty">No watchlist items.</div>';
...
async function loadLive(){
  if(!API_BASE){render(FALLBACK_REPORT);showStatus('V2 frontend installed. Configure the Worker URL in api-config.js to enable live research.');return}
  try{const [report,watch]=await Promise.all([api('/api/report'),api('/api/watchlist')]);render(report,watch);showStatus('Live research backend connected.')}catch(e){render(FALLBACK_REPORT);showStatus(`Backend unavailable: ${e.message}`,true)}
}
async function refresh(){
  const btn=document.querySelector('#refreshBtn');btn.disabled=true;btn.classList.add('busy');
  if(!API_BASE){showStatus('Configure the secure API URL before running live research.',true);btn.disabled=false;btn.classList.remove('busy');return}
  try{showStatus('Researching the next FOC window… this can take a little while.');const report=await api('/api/refresh',{method:'POST'});const watch=await api('/api/watchlist');render(report,watch);showStatus('New FOC Heat Report saved to research history.')}catch(e){showStatus(`Refresh failed: ${e.message}`,true)}finally{btn.disabled=false;btn.classList.remove('busy')}
}
...
render(FALLBACK_REPORT); loadLive();
